...
export default function App() {
  ...
  // Todas as disciplinas agrupadas por turma
  const disciplinas = {
    ...
    // Radiologia TRN2, TRN3 (Módulo 1,2,3)
    TRN2: [
  ...
  // Preencher turmas vazias com os mesmos dados da primeira turma do grupo
  ['TEM3','TEM4','TEM5','TEN2','TEN3','TEN4','TEN5','TEN6','TEN7'].forEach(t=>{
    disciplinas[t] = disciplinas['TEM2'];
  });
  ['TRN3'].forEach(t=>{
    disciplinas[t] = disciplinas['TRN2'];
  });

  useEffect(() => setTurmas(Object.keys(disciplinas)), []);

  const fazerLogin = () => { if(senhaAdmin===SENHA_PADRAO) setAutenticado(true); else alert('Senha incorreta!'); };
  ...
  const adicionarAluno = () => {
    ...
    disciplinas[turmaSelecionada].forEach(d=>{
      disciplinasAluno[d.nome] = { ...d, nota:'', faltas:'', aprovado:'', notaEstagio:'' };
    });
                ...
                <table className="w-full border border-gray-300 border-collapse">
                  <thead className="bg-gray-200">
                    <tr>
                      ...
                      <th className="border p-1">Nota Estágio</th>
                    </tr>
                  </thead>
                  <tbody>
                    {Object.entries(notasEncontradas.disciplinas).map(([nome, d])=>(
                      <tr key={nome}>
                        ...
                        <td className="border p-1">{d.notaEstagio}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                      ...
                      <tbody>
                        {Object.entries(aluno.disciplinas).map(([nome, d])=>(
                          <tr key={nome}>
                            ...
                            <td className="border p-1">{d.cargaEstagio||'-'}</td>
                            <td className="border p-1">
                              <input type="text" value={d.notaEstagio} onChange={e=>atualizarDisciplina(idx,nome,'notaEstagio',e.target.value)}
                                className="w-full px-1 py-0.5 border rounded"/>
                            </td>
                          </tr>
                        ))}
